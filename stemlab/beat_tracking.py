...
# the start of every run, and the file does not change between them. This is a
# record of work already done, never a substitute for it: the size and mtime it
# was taken over must still hold, and the digest it names must be the one the
# spec pins, or the checkpoint goes back through the hash.
...
def _record_verification(checkpoint: Path, info: os.stat_result, digest: str) -> None:
    ...
    after: a checkpoint rewritten while it was being read carries a newer mtime
    than this, and is hashed again rather than accepted on a digest taken over
    ...
    try:
        ...
        # A model directory that will not take the record costs later runs the
        # hash they would have skipped. It must not cost this one the model,
        # which is verified either way.
        try:
...
def _candidate_model_directories() -> list[Path]:
    ...
    if local:
        candidates.append(Path(local) / "FI-STEM" / "Models" / "BeatThis")
    return candidates
