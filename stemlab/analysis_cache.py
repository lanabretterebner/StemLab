...
def managed_analysis_dir() -> Path:
    """Return FI-STEM's private per-user analysis directory."""
    ...
    if local:
        return Path(local) / "FI-STEM" / "Analysis"
    return Path.home() / ".fi-stem" / "analysis"
...
def cleanup_stale_midi_drag_files(max_age_days: int = 7) -> int:
    """Delete only old temporary MIDI files in FI-STEM's managed drag directory."""
