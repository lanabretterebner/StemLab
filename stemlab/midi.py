...
def metadata_path_for(midi_path: str | Path) -> Path:
    path = Path(midi_path)
    return path.with_suffix(".fi-stem-midi.json")
...
def write_transcription(path: str | Path, transcription: MidiTranscription) -> Path:
    """Atomically persist FI-STEM's internal MIDI object."""
    path = Path(path)
...
def write_midi(
    ...
    *,
    ...
    track.append(mido.MetaMessage("track_name", name=f"FI-STEM {stem_type}", time=0))
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Convert one separated FI-STEM stem to MIDI.")
