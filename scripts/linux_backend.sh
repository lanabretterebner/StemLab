#!/usr/bin/env bash
#
# Install the FI-STEM separation backend on Linux - no venv, no system Python.
#
# Downloads a relocatable CPython (python-build-standalone), installs FI-STEM
# and its ML dependencies into it, and records the result where the plugin
# and Standalone app auto-discover it. After this script finishes, FI-STEM
# works with zero further configuration.
#
#   Engine   ~/.local/share/FI-STEM/Engine        ($XDG_DATA_HOME override)
#   Pointer  ~/.config/FI-STEM/portable_engine_path.txt
#
# Usage:
#   ./scripts/linux_backend.sh              # auto-detect GPU -> cuda/rocm/cpu
#   ./scripts/linux_backend.sh --cpu        # force CPU-only torch (smallest)
#   ./scripts/linux_backend.sh --cuda       # force NVIDIA CUDA torch
#   ./scripts/linux_backend.sh --rocm       # force AMD ROCm torch
#   ./scripts/linux_backend.sh --xpu        # force Intel GPU (XPU) torch
#   ./scripts/linux_backend.sh --dest DIR   # custom Engine location
#   ./scripts/linux_backend.sh --reinstall  # rebuild the Engine from scratch
#   ./scripts/linux_backend.sh --no-pointer # build-only: skip the discovery
...
[[ -n "$DEST" ]] || DEST="$DATA_HOME/FI-STEM/Engine"
...
echo "Installing the FI-STEM backend (torch: $TORCH_FLAVOR)..."
...
if [[ $WRITE_POINTER -eq 1 ]]; then
    mkdir -p "$CONFIG_HOME/FI-STEM"
    printf '%s\n' "$PYTHON" > "$CONFIG_HOME/FI-STEM/portable_engine_path.txt"

    cat <<EOF

FI-STEM backend installed.

  Engine:  $DEST
  Pointer: $CONFIG_HOME/FI-STEM/portable_engine_path.txt
...
FI-STEM backend assembled (no discovery pointer written).
