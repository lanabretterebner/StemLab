#!/usr/bin/env bash
#
# Build the FI-STEM VST3 + Standalone targets on Linux.
#
...
DND_PATCH="$REPO_ROOT/scripts/juce-linux-dnd.patch"
...
if [[ -f "$DND_PATCH" && -f "$DND_SOURCE" ]]; then
    if dnd_patch_in_place; then
        ...
        if [[ $JUCE_TREE_IS_OURS -eq 1 ]]; then
            ...
            echo "$JUCE_SOURCE carries an older FI-STEM drag-and-drop patch." >&2
...
ARTEFACTS="$BUILD_DIR/FIStemPlugin_artefacts/$BUILD_TYPE"
[[ -d "$ARTEFACTS" ]] || ARTEFACTS="$BUILD_DIR/FIStemPlugin_artefacts"

echo
echo "Build complete."
echo
if [[ -d "$ARTEFACTS/VST3/FI-STEM.vst3" ]]; then
    echo "  VST3:       $ARTEFACTS/VST3/FI-STEM.vst3"
fi

if [[ -f "$ARTEFACTS/Standalone/FI-STEM" ]]; then
    echo "  Standalone: $ARTEFACTS/Standalone/FI-STEM"
fi
echo
echo "The plugin needs the Python engine to separate anything. To install it:"
echo "  $REPO_ROOT/scripts/linux_backend.sh"
