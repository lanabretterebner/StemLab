...
@pytest.mark.parametrize(
    "script",
    ["scripts/setup_dev.ps1", "build_portable_windows.ps1", "build_installer_windows.ps1"],
)
...
def test_backend_output_suffixes_do_not_collide():
    ...
    command = (
        f". '{helper}'; "
        "'nvidia','cpu','amd' | ForEach-Object { (Get-FIStemBackendConfiguration $_).Suffix }"
    )
    result = subprocess.run(
...
@pytest.mark.parametrize(
    "script", ["build_portable_windows.ps1", "build_installer_windows.ps1"]
)
